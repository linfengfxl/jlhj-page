...
<ul>
  <li>
      <div v-bind:class="{'tv-node':1,'select':model.select}" :style="style" @click="model.doEvent && model.doEvent('select',$event)"> 
          <span v-bind:class="{'tv-arrow':1,'open':model.open,'close':!model.open,'empty':model.children.length==0}" @click="model.doEvent && model.doEvent('fold',$event)">
              <Icon type="arrow-right-b" class="arrow-close"></Icon>
              <Icon type="arrow-down-b" class="arrow-open"></Icon>
          </span>
          <span class="tv-title">{{model.title}}</span>
      </div>
